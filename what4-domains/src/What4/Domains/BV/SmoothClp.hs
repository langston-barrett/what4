{-|
Module      : What4.Domains.BV.SmoothClp
Copyright   : (c) Galois Inc, 2026
License     : BSD3

@SmoothClp@ is an abstract domain for bitvectors. An element is a tuple
@(start, stride, n)@ representing the @n + 1@ bitvectors visited by walking @n@
steps of size @stride@ from @start@ (mod @2^w@):

@
gamma((start, stride, n)) := { start + i * stride mod 2^w | 0 <= i <= n }
@

(Here, @gamma@ is the /concretization function/ in abstract interpretation.) We
call such a tuple a /progression/, in the sense of arithmetic progressions.

== Smooth strides

What characterizes this domain is how it represents the @stride@. Alongside the
exact @stride@ value, each progression caches the stride's /smooth part/: the
largest divisor of the form

@
2^k · m,   m = a squarefree product of primes from B = [3, 5, 7, 11, 13]
@

namely the 2-exponent @k@ (a 'lowestSetBit') and a @|B|@-bit presence mask for the
odd factors @m@ (the 'stridePow2' and 'oddFactorsMask' fields). The smooth part
always /divides/ the stride ('smoothPartDividesStride'), and equals it when the
stride is itself smooth.

The point of the cache is that the congruence-lattice operations read the smooth
part and become bit-operations on the @(k, mask)@ encoding, with no Euclidean gcd:

* a sound @gcd@ of two strides is a @min@ on the exponents and an @and@ on the
  masks; a sound @lcm@ is a @max@ and an @or@ ('smoothGcd'\/'smoothLcm') —
  exact when the operands are smooth, a sound divisor\/multiple otherwise;
* the meet's alignment of two cosets is a bounded prime-power CRT over the fixed
  basis ('smoothCrtAlign'), using only small-prime inverses.

Because these read only the smooth part, they cost the same whether or not the
stride is smooth, and 'mk' therefore stores the /exact/ requested stride rather
than rounding it: queries ('member', 'size', 'leqExact') and stride-preserving
operations stay exact, and the smooth approximation is confined to the lattice
combinators that genuinely need a small stride alphabet.

This module is the strides half of the reduced product
"What4.Domains.BV.SmoothClpBitwise", which pairs each progression with a bitwise
(known-bits) domain and keeps the two mutually 'reduce'd.
...
@
python3 -ic "$(awk '\/^def diagram\/,\/^    return/' src\/What4\/Domains\/BV\/SmoothClp.hs)"
@
...
-- TODO: Update haddocks
-- TODO: Complexity in terms of |B|
-- TODO: audit operations for odd-preservation (e.g., join)
...
module What4.Domains.BV.SmoothClp
  ...
  , smoothPartDividesAndSquarefree
  , smoothGcdDividesTrueGcd
  , smoothGcdExactOnSmooth
  , smoothLcmExactOnSmooth
  , smoothCrtAlignCorrect
  , smoothPartDividesStride
  -- ** Construction
  -- , correct_singleton
  , fromAscEltListMember
  , fromAscEltListToListExactNonWrapping
  , mkSmoothingSound
  , coarsenToSmoothSound
  -- ** Canonicalization
  , canonLossless
  , canonProper
  , canonUnique
  ...
  , correct_scale
  , correct_mul
  -- addSubSizeCorrect removed: the closed-form size scorer no longer matches
  -- the smoothed add\/sub result (orientation split deferred); see git history.
  -- addRobustClosedFormAgrees removed: add/sub are single-orientation now
  ...
  , addExact
  , subExact
  -- mulConstExact removed: scale-by-constant is no longer exact under smoothing
  -- (a non-smooth @k·stride@ is rounded down; plan #3 accepts mul precision loss).
  , udivConstExact
  , uremConstExact
  ...
  -- , liftForcedBitsRefinesSpec  -- disabled under smoothing; see its definition
  ...
  -- refineByBitsPreciseDominatesRoundTripNonSelfWrap removed: the cap on the
  -- forced-bit lift lowers strides precision, so it no longer dominates the
  -- toBitwise\/fromBitwise round-trip. See TODO(smoothclp-cap).
  , correct_refineBitsByStrides
  , refineBitsByStridesShrinks
  , refineBitsByStridesDominatesMeetToBitwise
  , correct_reduce
  , correct_reduceProject
  , reduceShrinksStrides
  , reduceShrinksBitwise
  , reduceConflictMeansEmpty
  , correct_reducePrecise
  , reducePreciseShrinksStrides
  , reducePreciseShrinksBitwise
  , reducePreciseConflictMeansEmpty
  , reducePreciseDominatesReduceStrides
  , reducePreciseDominatesReduceBitwise
  , reduceFixpointIdempotentStrides
  , reduceFixpointIdempotentBitwise
...
import           What4.Domains.Arithmetic (countTrailingZerosOr0, isPow2Natural, egcd)
...
data Domain (w :: Nat)
  = Domain
    ...
    , n :: !Natural
    , mask :: !Natural
    -- | The 2-exponent of the (smooth) stride: @stride = 2^stridePow2 ·
    -- oddProduct(oddFactorsMask)@, so @strideGcd = 2^stridePow2@. Cached for the
    -- lattice ops, which derive @gcd@\/@lcm@ of strides from the 'stridePow2' and
    -- 'oddFactorsMask' fields without a Euclidean gcd (see 'smoothGcd').
    , stridePow2 :: !Word
    -- | Presence bitmask of the squarefree odd part of the stride over the fixed
    -- basis 'basisPrimes': bit @i@ set iff @basisPrimes !! i@ divides the odd
    -- part. See 'smooth'.
    , oddFactorsMask :: !Word
    }
...
-- | /O(G(w))/. @solveLinearDiophantine a b c a_max b_max@ finds nonnegative
-- integers @x, y@ with @0 <= x <= a_max@, @0 <= y <= b_max@ such that
-- @a * x - b * y = c@. Assumes @a > 0@, @b > 0@, and @c /= 0@. Returns
-- 'Nothing' when no such pair exists; when one does, returns the lex-least
-- solution.
...
solveLinearDiophantine a b c a_max b_max
  | cr /= 0      = Nothing
  | t_upper < t  = Nothing
  | otherwise    = Just (nn * cq + bq * t, mm * cq + aq * t)
  where
    (g, nn, mm) = eGCD a (-b)
    (cq, cr)    = c `quotRem` g
    aq          = a `quot` g
...
-- such that @n * a + m * b = g@ and @g >= 0@. Delegates to the GMP-backed
-- 'egcd'.
...
ceilDivPos x y = assert (y > 0) (Prelude.negate (Prelude.negate x `div` y))
{-# INLINE ceilDivPos #-}
...
-- Smooth-stride basis
--
-- A stride is /smooth/ when it has the form @2^k · m@ where @m@ is a squarefree
-- product of odd primes drawn from the fixed basis @B = 'basisPrimes'@. The 2-part
-- @2^k@ is free ('lowestSetBit'); the odd part @m@ is a @|B|@-bit presence
-- bitmask ('oddFactorsMask'). Under this encoding the congruence lattice becomes
-- bit-operations: @gcd@\/@lcm@ of two smooth strides are @2^min/max(k) ·
-- (mask_a .&./.|. mask_b)@ ('smoothGcd'\/'smoothLcm'), with no Euclidean gcd.
-- 'mk' is the single point that smooths an arbitrary requested stride into this
-- form (rounding the odd part /down/ to its largest squarefree-basis divisor,
-- which denses the coset and so over-approximates).

-- | The fixed odd-prime basis @B@. Bit @i@ of an 'oddFactorsMask' selects
-- @basisPrimes !! i@.
basisPrimes :: [Natural]
basisPrimes = [3, 5, 7, 11, 13]

-- | @|B|@: the number of basis primes, i.e. the bit-width of an 'oddFactorsMask'.
basisSize :: Int
basisSize = length basisPrimes

-- | @R = ∏ B@: the product of all basis primes. An odd integer's
-- largest squarefree-basis divisor is @gcd(·, R)@ (since @R@ is squarefree).
basisProduct :: Natural
basisProduct = product basisPrimes

-- | /O(1)/. The odd part denoted by a presence bitmask: the product of the
-- basis primes whose bit is set. @oddProduct 0 = 1@ (the empty product). A
-- precomputed @2^|B|@-entry table (the bitmask is the index), so the odd-part
-- @gcd@\/@lcm@ in 'smoothGcd'\/'smoothLcm' is an AND\/OR plus one array read.
-- Only the low @|B|@ bits index the table (higher bits never occur on a valid
-- 'oddFactorsMask'); they are masked off so the lookup is always in range.
oddProduct :: Word -> Natural
oddProduct msk = oddProductTable Array.! (msk Bits..&. oddProductMaxIx)

-- | The largest valid 'oddFactorsMask' (all @|B|@ basis bits set), i.e. the top
-- index of 'oddProductTable'.
oddProductMaxIx :: Word
oddProductMaxIx = (1 `shiftL` basisSize) - 1

-- | The @2^|B|@-entry @mask -> product@ table backing 'oddProduct'. A top-level
-- CAF, built once.
oddProductTable :: Array.Array Word Natural
oddProductTable =
  Array.listArray (0, oddProductMaxIx)
    [ product [ p | (i, p) <- zip [0 ..] basisPrimes, Bits.testBit m i ]
    | m <- [0 .. oddProductMaxIx] ]

-- | /O(M(w))/. The largest squarefree-basis divisor of @m@, as
-- @(mask, product)@ with @product == 'oddProduct' mask@ and @product@ dividing
-- @m@. The product is @gcd(m, 'basisProduct')@: since @R = ∏B@ is squarefree the
-- gcd keeps each basis prime to at most the first power, so it is exactly the
-- squarefree-basis part — a single (constant-modulus) gcd that touches the
-- possibly-large @m@ just once, rather than one @m@-sized division per basis
-- prime. The mask is then read off the /small/ product (@<= R@). Only at
-- ingestion ('strideFields'\/'mk'); never on the lattice hot path.
smoothPartOf :: Natural -> (Word, Natural)
smoothPartOf m = (msk, prod)
  where
    prod = Prelude.gcd m basisProduct
    msk  = List.foldl' step 0 (zip [0 ..] basisPrimes)
    step acc (i, p) = if prod `mod` p == 0 then Bits.setBit acc i else acc

-- | /O(A(w))/. The cached smooth-stride fields @(stridePow2, oddFactorsMask)@
-- for a stride. For a /smooth/ stride this satisfies the 'smooth' invariant; for
-- the non-smooth @2^w − t@ reverse orientation built only inside 'canonicalize'
-- it records the 2-exponent and the squarefree-basis /part/ of the odd factor,
-- deterministically.
strideFields :: Natural -> (Word, Word)
strideFields st = (k, msk)
  where
    k        = fromIntegral (countTrailingZerosOr0 (toInteger st))
    oddPart  = st `shiftR` fromIntegral k
    (msk, _) = smoothPartOf oddPart

-- | Build a 'Domain' record from its set-defining fields, deriving the cached
-- 'stridePow2'\/'oddFactorsMask' from the stride via 'strideFields'. The single
-- point at which the cached fields are populated.
mkRaw :: Natural -> Natural -> Natural -> Natural -> Domain w
mkRaw s st nv m =
  Domain { start = s, stride = st, n = nv
         , stridePow2 = k, oddFactorsMask = om, mask = m }
  where (k, om) = strideFields st
{-# INLINE mkRaw #-}

-- | The smooth-stride invariant: @stride == 2^stridePow2 · oddProduct
-- oddFactorsMask@ with @oddFactorsMask@ within basis range. This is exactly the
-- statement that @stride@ is smooth. /Distinct from/ 'proper': 'canonicalize'
-- may produce a 'proper' but non-'smooth' representative (its @2^w − t@ reverse
-- orientation is generally not smooth), so this is asserted only on 'mk' output
-- and the operation results that flow through 'mk'.
smooth :: Domain w -> Bool
smooth Domain{stride, stridePow2, oddFactorsMask} =
  oddFactorsMask < (1 `shiftL` basisSize)
  && stride == (1 `shiftL` fromIntegral stridePow2) * oddProduct oddFactorsMask

-- | /O(A(w))/. The gcd of two smooth strides, read off their cached fields:
-- @2^min(k_a,k_b) · ∏(B_a ∩ B_b)@ — a @min@ on the 2-exponents and an AND on the
-- presence masks, then one 'oddProduct'. No Euclidean gcd. Exact when both
-- strides are smooth ('smoothGcdExactOnSmooth'); a sound common divisor in
-- general ('smoothGcdDividesTrueGcd').
smoothGcd :: (Word, Word) -> (Word, Word) -> Natural
smoothGcd (ka, ma) (kb, mb) =
  (1 `shiftL` fromIntegral (min ka kb)) * oddProduct (ma Bits..&. mb)

-- | /O(A(w))/. The lcm of two smooth strides, read off their cached fields:
-- @2^max(k_a,k_b) · ∏(B_a ∪ B_b)@ — a @max@ on the 2-exponents and an OR on the
-- presence masks. Exact when both strides are smooth; the result is itself
-- smooth.
smoothLcm :: (Word, Word) -> (Word, Word) -> Natural
smoothLcm (ka, ma) (kb, mb) =
  (1 `shiftL` fromIntegral (max ka kb)) * oddProduct (ma Bits..|. mb)

-- ------------------------------------------------------------------
-- Smooth-basis CRT (the meet's alignment)
--
-- The Chinese Remainder Theorem (CRT): if @m_1, ..., m_t@ are pairwise coprime
-- moduli, then for any target residues @r_1, ..., r_t@ the system @x ≡ r_i (mod
-- m_i)@ has a unique solution @x@ modulo @M = m_1 ··· m_t@. Recovering that @x@
-- from its residues is "recombination"; Garner's method does it incrementally,
-- one modulus at a time, needing a single modular inverse per modulus.
--
-- The meet of two arithmetic progressions is a CRT problem. The intersection of
-- @{x : x ≡ sA (mod ta)}@ and @{x : x ≡ sB (mod tb)}@ is @{x : x ≡ r (mod
-- lcm(ta,tb))}@, where @r@ is the CRT solution, which exists iff @sA ≡ sB (mod
-- gcd(ta,tb))@. For general moduli the recombination needs an extended-Euclid
-- inverse modulo each prime power, the G-class cost that 'arcMeetClosed'
-- otherwise pays through 'solveLinearDiophantine'.
--
-- Smoothness is exactly what makes the recombination cheap. A smooth stride
-- @2^k · ∏ B@ factors into pairwise coprime prime powers that are all either the
-- single 2-power @2^k@ or a distinct basis prime @p ≤ 13@ (the odd part is
-- squarefree, so no odd prime occurs to a power above 1). Hence:
--
--   * there are at most @1 + |B| = 6@ CRT components, all known up front;
--   * every odd modulus is a fixed small prime, whose inverse is a bounded
--     table-style search ('invModSmallPrime'), not an extended-Euclid solve; and
--   * taking the 2-power first as the CRT base means the one modulus that would
--     otherwise need a Hensel lift (an inverse mod @2^k@) is never inverted.
--
-- The whole alignment is then a handful of @w@-bit multiplies and small-prime
-- inverses (M\/A-class), never a Euclidean gcd (G). It is the lattice analogue of
-- the gcd\/lcm-to-bit-ops move that 'smoothGcd'\/'smoothLcm' play for the stride
-- arithmetic.
-- ------------------------------------------------------------------

-- | /O(M(w))/. Given two smooth strides @ta = 2^ka · oddProduct ma@ and
-- @tb = 2^kb · oddProduct mb@ (each passed by its cached fields, so the first
-- argument is @(ka, ma)@ and the third @(kb, mb)@) and two starts @sA@, @sB@,
-- returns the smallest nonnegative integer @r@ with @r ≡ sA (mod ta)@ and
-- @r ≡ sB (mod tb)@ — the unique such residue in @[0, lcm(ta,tb))@ — or 'Nothing'
-- when no such @r@ exists (@sA ≢ sB (mod gcd(ta,tb))@).
--
-- The result is correct only when @ta@ and @tb@ are genuinely 'smooth'; callers
-- must guard with 'smooth'. Validated for all smooth inputs by
-- 'smoothCrtAlignCorrect'. (See the section comment above for why smoothness
-- turns the recombination into bounded small-prime work with no Euclidean gcd.)
smoothCrtAlign ::
  (Word, Word) -> Natural ->   -- ^ @(ka, mask_a)@ and @sA@
  (Word, Word) -> Natural ->   -- ^ @(kb, mask_b)@ and @sB@
  Maybe Natural
smoothCrtAlign (ka, ma) sA (kb, mb) sB
  | (toInteger sB - toInteger sA) `Prelude.mod` toInteger g /= 0 = Nothing
  | otherwise = Just (fst (List.foldl' step (r0, m0) comps))
  where
    -- Solvable iff @sA ≡ sB@ mod the (exact, on smooth strides) gcd.
    g  = smoothGcd (ka, ma) (kb, mb)
    -- The 2-power is the CRT base; the binding side has the larger 2-exponent
    -- (the smaller side's congruence is implied by the @g@-divisibility check).
    m0 = 1 `shiftL` fromIntegral (max ka kb)
    r0 = (if ka >= kb then sA else sB) `Prelude.mod` m0
    -- One coprime component per basis prime present in either odd part. The
    -- binding residue uses @sA@ when @p | ta@ (the shared case already agrees).
    union = ma Bits..|. mb
    comps = [ (p, (if Bits.testBit ma i then sA else sB) `Prelude.mod` p)
            | (i, p) <- zip [0 ..] basisPrimes, Bits.testBit union i ]
    -- Garner step: lift @x@ (a solution mod @bigM@) to a solution mod
    -- @bigM · mi@ that is also @≡ ri (mod mi)@. @bigM@ is coprime to @mi@.
    step (x, bigM) (mi, ri) =
      let !inv'  = invModSmallPrime mi (bigM `Prelude.mod` mi)
          !diff  = (ri + mi - (x `Prelude.mod` mi)) `Prelude.mod` mi
          !tcoef = (diff * inv') `Prelude.mod` mi
      in (x + bigM * tcoef, bigM * mi)

-- | /O(A(w))/. The modular inverse of @a@ modulo a small prime @p@ (a
-- 'basisPrimes' member, so @p ≤ 13@). Requires @gcd(a, p) == 1@ (i.e.
-- @a `mod` p /= 0@).
invModSmallPrime :: Natural -> Natural -> Natural
invModSmallPrime p a =
  -- A bounded linear search over @[1, p)@: no Euclidean gcd, and constant-bounded
  -- by the largest basis prime, so it stays A-class.
  assert (a `Prelude.mod` p /= 0) $
  head [ y | y <- [1 .. p - 1], (a * y) `Prelude.mod` p == 1 ]

-- NOTE: 'smoothGcd'\/'smoothLcm'\/'smoothCrtAlign' are the A\/M-class replacements
-- for the G-class integer @gcd@\/@lcm@\/Diophantine solve on the lattice hot path
-- (the domain's thesis), now that the default 'add'\/'sub'\/'mul' are
-- single-orientation (every operand smooth). They are wired in:
-- 'addSubStrideAndSteps' uses 'smoothGcd' (so 'add'\/'sub' are M(w), not G(w));
-- 'arcMeetClosed' uses 'smoothLcm' for the combined stride and 'smoothCrtAlign'
-- for the coprime-odd-part alignment (so meet and the @assume*@ family are M(w),
-- not G(w), on smooth strides); and 'mul' uses 'smoothCornerProductStride' (the
-- factor-mask corner gcd) rather than the Euclidean 'cornerProductStride'. See
-- 'smoothGcdExactOnSmooth'\/'smoothLcmExactOnSmooth'\/'smoothCrtAlignCorrect' for
-- the exactness that makes the substitutions sound. (The overflow-flag
-- @add*@\/@sub*@\/@mul*@ variants are likewise M(w): 'addPieces'\/'subPieces' use
-- 'smoothGcd' and 'mulPieces' uses 'smoothCornerProductStride'.) The G-class
-- sites that remain are reached only off the smooth path: 'arcMeetClosed' falls
-- back to 'solveLinearDiophantine' for the non-smooth @2^w − t@ reverse
-- orientation, and the Euclidean 'cornerProductStride' is retained for the
-- precise corner machinery ('mulCorners'\/'mulCutUS', a future @mulPrecise@).
...
-- lowest set bit of @stride@, which is the cached @2^stridePow2@ (a field read,
-- not even a bit-trick: 'strideFields' pins @stridePow2@ to the stride's count of
-- trailing zeros on every construction).
strideGcd :: Domain w -> Natural
strideGcd Domain{stridePow2} = 1 `shiftL` fromIntegral stridePow2
{-# INLINE strideGcd #-}
...
mk ::
  ...
  Natural ->
  Domain w
mk w s st nn = mkCanon w s st nn
{-# INLINE mk #-}

-- | /O(A(w))/. Like 'mk' but /without/ smoothing the stride: it applies only the
-- singleton\/full-coset canonicalization and builds the record, preserving the
-- exact requested orbit. The result is 'proper' but /not necessarily/ 'smooth'.
--
-- This is the constructor for the transient non-smooth representatives that the
-- design deliberately allows (plan #2\/#5): the @2^w − t@ reverse orientation
-- ('reverseD') and the @2·t@ parity split ('psplit'), whose strides are
-- generally not smooth and whose /sets/ must be preserved exactly. Such values
-- are never stored as smooth domains; any operation consuming them re-smooths
-- its result through 'mk'.
mkCanon ::
...
-- | /O(M(w))/. Coarsen a progression to one with a /smooth/ stride: round the
-- odd part of the stride down to its largest squarefree-basis divisor and
-- rescale the step count to keep the same spanned arc. The result is a superset
-- ('coarsenToSmoothSound') with a smooth stride, so the A-class lattice helpers
-- ('smoothLcm', 'smoothCrtAlign') are exact on it. Identity on an
-- already-'smooth' input.
--
-- This is the widening that the loose meet ('pseudoMeet') applies to its
-- operands so that its A-class kernel is sound: a smooth-strided lower bound of
-- the coarsened operands is a sound over-approximation of the true intersection.
coarsenToSmooth :: NatRepr w -> Domain w -> Domain w
coarsenToSmooth w c
  | smooth c  = c
  | otherwise = mkCanon w (start c) stSmooth nnSmooth
  where
    -- Keep the 2-part @g@; round the odd part down to its smooth part; rescale
    -- the step count by @stride \/ stSmooth@ so the arc @[start, start + n·stride]@
    -- is preserved (a denser, over-approximating coset).
    st       = stride c
    g        = lowestSetBit st
    stSmooth = g * snd (smoothPartOf (st `divByPow2` g))
    nnSmooth = clampToOrbit (mask c) stSmooth (n c * (st `Prelude.div` stSmooth))
...
-- common module that 'SmoothClp' can import (e.g. by adding a dep from
...
fromJustUnsafe loc = \case
  Just x  -> x
  Nothing -> error ("What4.Domains.BV.SmoothClp: " ++ loc ++ ": Nothing")
...
-- "[*.*.*.*.*.*.*.*.]  = [0,2,4,6,8,10,12,14]"
--
-- 'add' is /single-orientation/: it does not try both the stride and its
-- modular negation @2^w - t@. Reversing to @2^w - t@ produces a non-smooth
-- stride, which would coarsen the result coset and force the Euclidean gcd
-- rather than the A-class 'smoothGcd'. Orientation-robustness belongs in a
-- future @addPrecise@.
add :: (1 <= w) => NatRepr w -> Domain w -> Domain w -> Domain w
add w = addRaw w

-- | /O(M(w))/. The single-orientation addition kernel; called by 'add'.
...
-- "[.*******........]  = [1,2,3,4,5,6,7]"
--
-- Like 'add', 'sub' is single-orientation: reversing an operand to
-- @2^w - t@ produces a non-smooth stride, blocking 'smoothGcd'.
-- Orientation-robustness belongs in a future @subPrecise@.
sub :: (1 <= w) => NatRepr w -> Domain w -> Domain w -> Domain w
sub w = subRaw w

-- | /O(M(w))/. The single-orientation subtraction kernel; called by 'sub'.
...
addSubStrideAndSteps na sa nb sb = (d, n')
  where
    -- The result stride is @gcd(sa, sb)@. SmoothClp computes it with the
    -- A-class 'smoothGcd' (a @min@ of 2-exponents and an AND of odd-factor
    -- masks) instead of a G-class Euclidean gcd. This is /exact/ when @sa@, @sb@
    -- are smooth — which they always are here, since 'add'\/'sub' are
    -- single-orientation, so both operands come straight from 'mk'. ('smoothGcd'
    -- stays a sound common divisor even off the smooth path
    -- ('smoothGcdDividesTrueGcd'), so @sa \`div\` d@ below is always exact.)
    d = case (na, nb) of
          (0, 0) -> 1
          (0, _) -> sb
          (_, 0) -> sa
          _      -> smoothGcd (strideFields sa) (strideFields sb)
...
reverseD w c@Domain{stride, n = nn, mask} =
  assert (proper c) $
  -- 'mkCanon', not 'mk': the reversed stride @2^w − stride@ is generally not
  -- smooth, and reversal must denote the /same/ set ('reverseDSameSet'), so it
  -- must not be smoothed. The result is a transient non-smooth representative
  -- (plan #2); the orientation-robust callers re-smooth their final result.
  mkCanon w (end c) (modNeg mask stride) nn
  ...
  in if nn == 0 || nn + 1 == orbitLen c || t' == 0
       then [c]
         ...
         let !nEven = nn `Prelude.div` 2
             ...
             !sOdd  = (s + t) .&. m
         -- 'mkCanon', not 'mk': the doubled stride @2·t@ is generally not smooth,
         -- and the two halves must /exactly/ partition @c@ ('psplitCovers',
         -- 'psplitPartitions'), so they must not be smoothed. They are transient
         -- pieces consumed by 'psplitOp2', which re-smooths its result.
         in [mkCanon w s t' nEven, mkCanon w sOdd t' nOdd]
...
-- | /O(M(w))/. Multiplication.
--
-- Uses the A-class 'smoothCornerProductStride' (read off the operands' smooth
-- stride factors) for the result coset stride, so no Euclidean gcd is ever run;
-- the cost is the M-class corner-product multiplies. Concretely this is
-- 'mulCorners' on the single-orientation operands, /without/ the 'mulCutUS'
-- refinement: the cut sharpens some wrapping products but routes through the
-- Euclidean 'cornerProductStride', which would pull 'mul' back up to G(w). The precise
-- corner machinery ('mulCorners' with the Euclidean 'cornerProductStride',
-- 'mulCutUS') is retained for a future @mulPrecise@, exactly as the
-- 'orientRobust' machinery is kept for a future @addPrecise@.
mul :: (1 <= w) => NatRepr w -> Domain w -> Domain w -> Domain w
mul w = mulRaw w

-- | /O(M(w))/. The multiplication kernel: 'mulCorners' specialized to the
-- A-class 'smoothCornerProductStride'. Sound (every concrete product is a
-- member); equal to the precise corner product when the integer corner gcd is
-- below @2^w@ (its smoothing and the pre-mod smoothing then agree), coarser
-- otherwise and on wrapping operands (no cut).
--
-- References:
--
-- * WI 3.2 Analysing expressions
...
mulRaw w = mulCornersWith smoothCornerProductStride w
...
mulCorners = mulCornersWith cornerProductStride

-- Like 'mulCorners' but parameterized over the coset-stride function (see
-- 'mulFromCornersWith'); 'mulFast' instantiates it with the A-class
-- 'smoothCornerProductStride'.
mulCornersWith ::
  (1 <= w) =>
  (Domain w -> Domain w -> Integer -> Integer -> Integer) ->
  NatRepr w -> Domain w -> Domain w -> Domain w
mulCornersWith strideFn w a b
  | n a == 0 = scaleSingleton w (start a) b
  | n b == 0 = scaleSingleton w (start b) a
  | otherwise =
      let !(uAl, uAh) = unsignedArc a
          !(uBl, uBh) = unsignedArc b
          !rUU = mulFromCornersWith strideFn w a b uAl uAh uBl uBh
      in case (midUpper a, midUpper b) of
           (False, False) -> rUU
           (True,  False) ->
             let !(zAl, zAh) = zboundsArc a
                 !rZU = mulFromCornersWith strideFn w a b zAl zAh uBl uBh
             in minBy size rUU rZU
           (False, True ) ->
             let !(zBl, zBh) = zboundsArc b
                 !rUZ = mulFromCornersWith strideFn w a b uAl uAh zBl zBh
           ...
           (True,  True ) ->
             let !(zAl, zAh) = zboundsArc a
                 ...
                 !rUZ = mulFromCornersWith strideFn w a b uAl uAh zBl zBh
                 !rZU = mulFromCornersWith strideFn w a b zAl zAh uBl uBh
                 !rZZ = mulFromCornersWith strideFn w a b zAl zAh zBl zBh
...
mulFromCorners = mulFromCornersWith cornerProductStride

-- Like 'mulFromCorners' but parameterized over the coset-stride function, so
-- 'mulFast' can swap in the A-class 'smoothCornerProductStride' for the G-class
-- 'cornerProductStride'. The 'clpStepBound'\/'arcStepBound' step counts stay
-- sound for any @strideFn@ result that divides the true corner-product gcd (a
-- smaller stride only inflates the bound, which 'clampToOrbit' then caps).
mulFromCornersWith ::
  (1 <= w) =>
  (Domain w -> Domain w -> Integer -> Integer -> Integer) ->
  NatRepr w -> Domain w -> Domain w ->
  Integer -> Integer -> Integer -> Integer -> Domain w
mulFromCornersWith strideFn w a b al ah bl bh =
  let !(lo, hi) = cornerArc al ah bl bh
      !dIntI = strideFn a b al bl
...
-- The A-class smooth replacement for 'cornerProductStride': a sound smooth
-- divisor of @gcd(t1·|bl|, t2·|al|, t1·t2)@ computed from the operand stride
-- /factors/, with NO Euclidean gcd. Each corner product @t·c@ decomposes as a
-- 2-exponent @v2(t) + v2(c)@ and a basis-prime presence mask @mask(t) ∪
-- smoothPart(|c|)@ (a prime divides @t·c@ iff it divides either factor); the
-- gcd's 2-exponent is the @min@ of those exponents and its basis primes are the
-- intersection (@.&.@) of the masks. The result @2^v · oddProduct(common)@ is
-- exactly @smooth(g)@ (the squarefree-basis smoothing of the true integer gcd
-- @g@), so it divides @g@ and is a sound (over-approximating) coset stride; it
-- is what 'cornerProductStride' + 'mk' would produce when @g < 2^w@, but it
-- never forms @t1·t2@\/@t1·bl@ and never runs Euclid, turning the G-class mul
-- stride into an A-class one. See 'mulFast'.
smoothCornerProductStride ::
  Domain w -> Domain w -> Integer -> Integer -> Integer
smoothCornerProductStride a b al bl =
  case terms of
    []       -> 1   -- both operands singletons; stride is irrelevant (mulFast
                    -- routes singletons through 'scaleSingleton' first)
    (t : ts) ->
      let !minv2 = Prelude.foldr (Prelude.min . fst) (fst t) ts
          !cmask = Prelude.foldr ((Bits..&.) . snd) (snd t) ts
      in (1 `shiftL` fromIntegral minv2) * toInteger (oddProduct cmask)
  where
    -- Present corner-product terms as (2-exponent, basis-presence mask):
    --   d22 = t1·t2   (both operands step)
    --   d12 = t1·|bl| (a steps, anchor bl /= 0)
    --   d21 = t2·|al| (b steps, anchor al /= 0)
    terms :: [(Word, Word)]
    terms =
      [ (stridePow2 a + stridePow2 b, oddFactorsMask a Bits..|. oddFactorsMask b)
      | n a /= 0, n b /= 0 ] ++
      [ (stridePow2 a + ctz bl, oddFactorsMask a Bits..|. smoothMaskOf bl)
      | n a /= 0, bl /= 0 ] ++
      [ (stridePow2 b + ctz al, oddFactorsMask b Bits..|. smoothMaskOf al)
      | n b /= 0, al /= 0 ]
    ctz x = fromIntegral (countTrailingZerosOr0 (Prelude.abs x)) :: Word
    smoothMaskOf x =
      let !nx = fromInteger (Prelude.abs x) :: Natural
          !k  = countTrailingZerosOr0 (toInteger nx)
      in fst (smoothPartOf (nx `shiftR` k))
...
-- with step @smoothGcd(stride ai, stride bj)@ (singletons skipped).
--
...
addPieces a b splitOp arcOf =
  ...
  , let (alo, ahi) = arcOf ai
        ...
        d = case (n ai, n bj) of
              ...
              -- A-class result stride: the pieces inherit the (smooth, on
              -- 'mk'-produced inputs) parent strides, so 'smoothGcd' on the cached
              -- @(stridePow2, oddFactorsMask)@ fields is the true gcd; off smooth
              -- it is a sound divisor (denser coset, still a superset). No Euclid.
              _      -> toInteger (smoothGcd (stridePow2 ai, oddFactorsMask ai)
                                             (stridePow2 bj, oddFactorsMask bj))
  ]
...
subPieces a b splitOp arcOf =
  ...
  , let (alo, ahi) = arcOf ai
        ...
        d = case (n ai, n bj) of
              ...
              (0, _) -> toInteger (stride bj)
              (_, 0) -> toInteger (stride ai)
              -- See 'addPieces': A-class 'smoothGcd' on the cached fields.
              _      -> toInteger (smoothGcd (stridePow2 ai, oddFactorsMask ai)
                                             (stridePow2 bj, oddFactorsMask bj))
  ]
...
-- 'smoothCornerProductStride'. Singletons fold their contribution into the
...
mulPieces _w a b splitOp arcOf =
  ...
  , let (al, ah) = arcOf ai
        ...
        -- A-class corner stride (matches the live 'mul'\/'mulRaw'): a smooth
        -- /divisor/ of the exact 'cornerProductStride' ('cornerProductStrideDivides'),
        -- so the result coset is denser (a superset) — sound for the
        -- overflow-flag over-approximation, with no Euclidean gcd.
        dInt     = smoothCornerProductStride ai bj al bl
  ]
...
-- | /O(M(w))/. Multiplication with the @nuw@ flag.
mulNuw ::
  (1 <= w) => NatRepr w -> Domain w -> Domain w -> Maybe (Domain w)
mulNuw w a b =
  assert (proper a) $ assert (proper b) $
...
-- | /O(M(w))/. Multiplication with the @nsw@ flag.
mulNsw ::
  (1 <= w) => NatRepr w -> Domain w -> Domain w -> Maybe (Domain w)
mulNsw w a b =
  assert (proper a) $ assert (proper b) $
...
-- | /O(M(w))/. Multiplication with both @nsw@ and @nuw@.
mulNswNuw ::
  (1 <= w) => NatRepr w -> Domain w -> Domain w -> Maybe (Domain w)
...
-- == Low-mask fast path
--
-- When @k@ is a /contiguous low mask/ @2^j - 1@, @k & y = y \`mod\` 2^j@, which
-- preserves the odd part of the stride (for @stride = 2^v · m@ with @v < j@ and
-- a non-wrapping truncated orbit). The generic 'fromForcedBits' path can only
-- emit power-of-two strides, so it discards that odd part; the cases below
-- route the exactly-representable results through 'mk' instead. This matters
-- for the @x & 0xFF@ \/ @mod 2^k@ field-extraction idiom. Note that on this
-- domain 'mk' /smooths/ the stride, so the low-mask result is sound and
-- dominates the fallback but is only exact when the truncated stride\'s odd
-- part is already smooth (see 'mkSmoothingSound'); the fallback handles the
-- wrap-without-saturation case.
andSingleton :: (1 <= w) => NatRepr w -> Natural -> Domain w -> Domain w
-- The fallback result's forced-bit picture is @(~k | zeros, k & ones)@ — a bit
-- is cleared wherever @k@ is @0@ or @c@ forces a @0@, set wherever @k@ is @1@
...
andSingleton w k c =
  assert (proper c) $
  if k .&. (k + 1) /= 0
    then fallback                        -- not a contiguous low mask
    else
      let !j    = popCount k             -- k == 2^j - 1
          !twoJ = 1 `shiftL` j :: Natural
          !g    = strideGcd c            -- 2^v
          !s0   = start c .&. k
          !tLow = stride c .&. k
          !nn   = n c
      in if g >= twoJ || nn == 0
           then mk w s0 1 0                                       -- (1) singleton
           else if nn + 1 >= twoJ `div` g
             then mk w (start c .&. (g - 1)) g (twoJ `div` g - 1) -- (2) full coset
             else if s0 + nn * tLow < twoJ
               then mk w s0 tLow nn                               -- (3) non-wrapping
               else fallback                                     -- (4) wrap, no saturate
  where
    !m              = mask c
    !(zeros, ones)  = forcedBits c
    !(_, cHi)       = operandRange c
    fallback        =
      fromForcedBits w ((m `Bits.xor` k) Bits..|. zeros, k Bits..&. ones) (0, cHi)
...
-- | /O(M(w))/. The single-pair shift-left kernel; see 'shl' for the
...
-- | /O(M(w))/. Sound /over/-approximation of the intersection of two progressions.
...
pseudoMeet w a b =
  ...
  case () of
    _ | leq a b -> Just a
      | leq b a -> Just b
      -- Genuine overlap: 'coarsenToSmooth' the operands so the A-class kernel
      -- ('smoothLcm') is sound, accepting a coarser (still sound) over-approximation
      -- on non-smooth strides. The containment short-circuits above keep the
      -- top-identity and idempotence laws exact; the lower-bound law holds for
      -- /smooth/ operands ('pseudoMeetLowerBound'). The exact (genuine lower
      -- bound) meet on non-smooth strides is 'pseudoMeetPrecise'\/'lowerBound'.
      | otherwise -> pseudoMeetStridesBy compactify w
                       (coarsenToSmooth w a) (coarsenToSmooth w b)
...
restrictToCoset w arith s d
  ...
  | otherwise =
      let lo = start arith
          ...
          -- Computed in plain integers (not reduced mod @2^w@ first): for an
          -- odd-bearing @d@ the residue mod @d@ is not wrap-invariant, so the
          -- caller must only pass a non-power-of-two @d@ when @arith@ does not
          -- wrap (then @[lo, end']@ is a genuine integer interval and this is
          -- exact). For a power-of-two @d | 2^w@ this agrees with the mod-@2^w@
          -- residue, so 'pseudoMeet'\'s wrap path (which passes 'strideGcd') is
          -- unaffected.
          off = fromInteger
                  ((toInteger s - toInteger lo) `Prelude.mod` toInteger d)
          lo' = lo + off
      in if lo' > end'
           then Nothing
...
arcMeetClosed w a b
  ...
  | otherwise =
      ...
      -- @lcm(stride a, stride b)@: the result coset stride. It must be a
      -- /multiple/ of both operand strides, or the intersection coset would step
      -- outside an operand (the meet would not be a subset of its inputs). When
      -- both operands are smooth, the A-class 'smoothLcm' (a @max@ of 2-exponents
      -- and an OR of odd-factor masks off the cached fields) is exactly that lcm.
      -- A non-smooth operand has no smooth multiple (nothing smooth is a multiple
      -- of e.g. @9 = 3^2@), so 'smoothLcm' would under-estimate and break the
      -- subset property; there we use the G-class integer 'lcmNat' instead. Either
      -- way @d@ is the exact lcm, so the meet is exact.
      let !d  | smooth a && smooth b =
                  smoothLcm (stridePow2 a, oddFactorsMask a)
                            (stridePow2 b, oddFactorsMask b)
              | otherwise = lcmNat (stride a) (stride b)
          !ta = stride a
          !tb = stride b
          !lo = max sA sB
          !hi = min endA endB
          -- Build the intersection coset at @theta@ with explicit stride @dd@.
          mkResultWith dd theta =
            let !steps = (hi - theta) `Prelude.div` dd
                ...
                !dd' = if dd > mask a then 1 else dd
            in Just (mk w (modMask a theta) dd' steps)
          -- First member of the coset @{s0 + k·tf}@ at or after @lo@ (@lo ≥ s0@
          -- holds since @lo = max sA sB@ and @s0 ∈ {sA, sB}@), clipped to @hi@.
          clipToFiner s0 tf =
            let !theta = s0 + ((lo - s0 + tf - 1) `Prelude.div` tf) * tf
            in if theta > hi then Nothing else mkResultWith tf theta
          -- @start b − start a@ is divisible by @m@ (the alignment test).
          alignedMod m = (toInteger sB - toInteger sA) `Prelude.mod` toInteger m == 0
      in if hi < lo then Nothing
         -- Equal anchors: the smallest common value is @lo@ itself, stride
         -- @lcm@; no solve needed (the original fast case).
         else if sA == sB then mkResultWith d lo
         -- Nested strides: when one stride divides the other (a cheap integer
         -- @mod@, not a gcd), the integer-arc intersection collapses to the
         -- /finer/ coset clipped to @[lo, hi]@ with NO Diophantine\/CRT solve.
         -- @ta | tb@ means @b@\'s coset @{≡ sB mod tb}@ already implies @{≡ sB
         -- mod ta}@, so the meet is just @b@ restricted to the overlap window,
         -- provided the anchors are congruent mod the coarser stride. (Smooth
         -- strides make nesting common: any two powers of two nest, as do
         -- @2^j·m₁ | 2^k·m₂@ whenever @m₁ | m₂@.) Genuinely coprime odd parts
         -- (e.g. 3 and 5) nest neither way and fall through to the solve.
         else if tb `Prelude.mod` ta == 0   -- ta | tb: b is the finer coset
                then if alignedMod ta then clipToFiner sB tb else Nothing
         else if ta `Prelude.mod` tb == 0   -- tb | ta: a is the finer coset
                then if alignedMod tb then clipToFiner sA ta else Nothing
         -- Coprime odd parts (e.g. 3 and 5): neither stride divides the other,
         -- so the alignment is a genuine Chinese-Remainder-Theorem (CRT) solve.
         -- On smooth strides 'smoothCrtAlign'
         -- solves it with bounded prime-power inverses (M\/A-class, no Euclid),
         -- returning the intersection coset's residue @r@ in @[0, lcm)@; we then
         -- take its first member at or after @lo@ and clip to @hi@. (This is
         -- exactly the value the lex-least Diophantine solution would give: the
         -- smallest common value @≥ lo@, bounded by @theta ≤ hi ⟺ i ≤ n a ∧
         -- j ≤ n b@.) The non-smooth @2^w − t@ reverse orientation, which only
         -- the canonicalize-internal callers can produce, falls back to the
         -- G-class 'solveLinearDiophantine' so the smooth-field decomposition is
         -- never trusted off its domain.
         else if smooth a && smooth b
                then case smoothCrtAlign (stridePow2 a, oddFactorsMask a) sA
                                         (stridePow2 b, oddFactorsMask b) sB of
                       Nothing -> Nothing
                       Just r ->
                         let !theta = if r >= lo
                                        then r
                                        else r + ((lo - r + d - 1) `Prelude.div` d) * d
                         in if theta > hi then Nothing else mkResultWith d theta
                else case solveLinearDiophantine
                            (toInteger ta) (toInteger tb)
                            (toInteger sB - toInteger sA)
                            (toInteger (n a)) (toInteger (n b)) of
                       Nothing -> Nothing
                       Just (i, _) ->
                         let !theta = sA + fromInteger i * ta
                         in if theta > hi
                              then Nothing
                              else mkResultWith d theta
  where
    sA = start a
    sB = start b
...
pseudoJoinStrides w a b =
  assert (proper a) $
  assert (proper b) $
  -- @deltaAbs = |start a − start b|@ as a /plain integer/ (not reduced mod
  -- @2^w@): the odd factors of @g@ must divide @start a − start b@ as integers
  -- to put both anchors on the same stride-@g@ coset, and reducing mod @2^w@
  -- first would inject a @2^w@ term that only the 2-power part divides. The
  -- absolute value is symmetric, which also keeps the join commutative.
  let !deltaAbs | start a >= start b = start a - start b
                | otherwise          = start b - start a
      !arc   = A.join (toArith a) (toArith b)
  in case fromArith w arc of
       Nothing  -> mk w 0 1 (mask a)  -- shouldn't happen on proper inputs
       Just dom ->
         -- Result coset stride @g = gcd(stride a, stride b, deltaAbs)@ kept
         -- smooth: the coset @{start a + k·g}@ contains both operands (so the
         -- join is a sound upper bound) because @g@ divides @stride a@, @stride
         -- b@, and @start a − start b@. The 2-power part is a 'min' of the two
         -- 2-exponents (further capped by @deltaAbs@\'s); the odd part is the
         -- basis primes shared by both strides (an AND of 'oddFactorsMask's) that
         -- also divide @deltaAbs@ — no Euclidean gcd. A singleton imposes no coset
         -- constraint (it lies on /every/ coset), so it contributes the other
         -- operand's fields.
         let (ka, ma) = strideFields (stride a)
             (kb, mb) = strideFields (stride b)
             !v2gcd = case (n a, n b) of
                        (0, 0) -> 0
                        (0, _) -> kb
                        (_, 0) -> ka
                        _      -> min ka kb
             !omGcd = case (n a, n b) of
                        (0, 0) -> 0
                        (0, _) -> mb
                        (_, 0) -> ma
                        _      -> ma Bits..&. mb
             (kd, md) = strideFields deltaAbs  -- 2-exponent + odd basis of @deltaAbs@
             !v  = if deltaAbs == 0 then v2gcd else min v2gcd kd
             !om = if deltaAbs == 0 then omGcd else omGcd Bits..&. md
             -- The odd factors are only sound when /neither operand/ wraps mod
             -- @2^w@ and the hull is a genuine integer interval (@start + n·stride
             -- <= mask@): otherwise an orbit crosses the @2^w@ seam, where the
             -- value jumps by a multiple of the 2-power part but of no odd factor,
             -- so an odd-bearing coset would no longer contain it. (These are the
             -- @start + n·stride@ orbit-wrap tests, not 'isSelfWrapping' which
             -- ignores @start@; they coincide with 'pseudoJoinUpperBound's
             -- non-wrap precondition.) When any wraps, keep just the
             -- (wrap-invariant) 2-power part.
             wraps c = start c + n c * stride c > mask c
             !safe = Prelude.not (wraps a || wraps b || wraps dom)
             !g | safe      = (1 `shiftL` fromIntegral v) * oddProduct om
                | otherwise = 1 `shiftL` fromIntegral v
         in case restrictToCoset w dom (start a) g of
...
-- 'toBitwise'\/'fromBitwise' (already exported); 'reduce' is the
-- /reduction operator/ that exchanges those projections to refine both
-- components in lockstep, in a single pass.
--
...
liftForcedBits w s (zeros, ones)
  ...
  where
    ...
    -- Full contiguous forced run, then /capped/ so the lifted stride
    -- @2^k · stride@ stays below @2^w@. Doubling past that point wraps mod
    -- @2^w@ and scrambles the (smooth) odd part of the stride, which 'mk' would
    -- then round down to a coarser, non-subset coset (e.g. @4·105 mod 256 =
    -- 4·41@, smoothed to @4@). Capping pins only the low forced bits the stride
    -- can hold smoothly; the higher forced bits are left to the bitwise
    -- component of the reduced product (which keeps them via @b@).
    --
    -- TODO(smoothclp-cap): investigate further. Open questions:
    --   * Precision cost: how often does the cap fire on realistic inputs, and
    --     how much strides-side precision is lost vs. the unbounded Strides
    --     lift? (The bitwise side recovers the high pins, but the strides orbit
    --     stays coarser; quantify via a precision-regression corpus.)
    --   * Does the coarser strides result still let 'reduceFixpoint' converge
    --     to the same fixed point (just in more rounds), or can the cap stall
    --     mutual refinement?
    --   * Is there a tighter sound option than dropping the high forced bits
    --     entirely — e.g. choosing a different smooth stride that still pins
    --     some of them, or representing the lifted window without densifying?
    --   * This is the same overflow-scrambles-the-odd-part phenomenon behind the
    --     'mk' smoothing precision loss generally; a shared treatment may exist.
    fullTwoK = if csf == 0 then hiSpan else lowestSetBit csf
    twoK     = capToSmooth fullTwoK
    capToSmooth t = if t * st <= m then t else capToSmooth (t `divByPow2` 2)
    k       = popCount (twoK - 1)
      ...
      | otherwise =
          ...
          in if Prelude.not (forcedZero || forcedOne)
               then Just c
                 ...
                 in if (stB `shiftL` 1) > m
                      -- Cap (matches the closed-form 'liftForcedBits'): doubling
                      -- the stride would reach\/exceed @2^w@ and scramble its
                      -- smooth odd part. Stop lifting here, leaving the
                      -- remaining forced bits to the bitwise side. (This
                      -- subsumes the old @newStride == 0@ singleton-collapse
                      -- case at @stB = 2^(w-1)@.)
                      --
                      -- TODO(smoothclp-cap): keep in sync with the closed-form
                      -- 'liftForcedBits' cap; see the investigation notes there.
                      then Just c
                      else
...
--
-- == Soundness and shrinkage
--
-- Each output component is a subset of the corresponding input
-- ('reduceShrinksStrides', 'reduceShrinksBitwise'); when @x@ lies in
-- both inputs, @x@ lies in both outputs ('correct_reduce').
--
-- == Comparison with 'reducePrecise'
--
-- Each component of 'reduce' is a superset of the corresponding
-- 'reducePrecise' component ('reducePreciseDominatesReduceStrides',
-- 'reducePreciseDominatesReduceBitwise'). Strictly weaker whenever
-- @b@\'s arc @[blo, bhi]@ carves a sub-arc tighter than its forced bits
-- suggest — most common when @b@ came from a comparison assumption like
-- @x <= k@. When @b@ is mostly mask-shaped (forced bits dominate the
-- information content), 'reduce' recovers most of 'reducePrecise'\'s
-- precision.
--
-- == Example
--
...
reduce w s b
  ...
  | otherwise = do
      s' <- liftForcedBits w s (knownZerosOnesNat b)
      let b' = B.meet b (projectBitwise w s')
      if B.isBottom b' then Nothing else Just (s', b')

-- | /O(M(w))/. The bitwise domain whose forced bits are exactly those held to a
-- known value across the whole strides orbit. This is the orbit→tnum direction
-- shared by 'reduce' and 'reduceProject'.
projectBitwise :: (1 <= w) => NatRepr w -> Domain w -> B.Domain w
projectBitwise w s
  -- Project each 'ssplit' piece with 'toBitwise' and join. Per-piece projection
  -- is bit-complete on a non-wrap piece, so the join is at least as tight as
  -- 'toBitwise' of the whole orbit, and strictly tighter on some wrapping orbits.
  -- A self-wrapping @s@ would have 'ssplit' collapse to the full coset (a
  -- superset of @s@), so project the original @s@ directly instead.
  | isSelfWrapping s = toBitwise s
  | otherwise =
      case ssplit w s of
        c : cs -> foldr (B.join . toBitwise) (toBitwise c) cs
        []     -> toBitwise s  -- unreachable: ssplit never returns []

-- | /O(M(w))/. Refine the bitwise component of a strides\/bitwise pair against
-- the strides orbit, leaving the orbit itself unchanged. Returns 'Nothing' iff
-- the refined bitwise component is empty (same @'B.isBottom' ⇒ 'Nothing'@
-- contract as 'reduce').
--
-- Always sound. It is as precise as the full 'reduce', and yields a result that
-- still satisfies the reduced-product invariant, precisely on pairs whose
-- bitwise component already leaves bit @ctz(stride)@ free — the 'add'\/'sub'
-- results and the feed-forward constructors for which 'SmoothClpBitwise' uses
-- it. It is /not/ valid where the orbit still needs refinement from the bitwise
-- side ('scale', 'mul', the bitwise ops). Soundness: 'correct_reduceProject'.
reduceProject ::
  (1 <= w) =>
  NatRepr w ->
  Domain w ->
  ...
  Maybe (Domain w, B.Domain w)
reduceProject w s b
  -- Only the orbit→tnum direction of 'reduce': meet @b@ with the orbit's
  -- 'projectBitwise' and skip 'liftForcedBits'. Lossless exactly when @b@ leaves
  -- bit @ctz(stride s)@ free, since the omitted lift would then have hit its
  -- @forced .&. 2^v == 0@ short-circuit and returned @s@ unchanged anyway.
  | B.isBottom b = Nothing
  | otherwise =
      let b' = B.meet b (projectBitwise w s)
      in if B.isBottom b' then Nothing else Just (s, b')
...
-- == Algorithm
--
-- 1. 'ssplit' @s@ at the unsigned pole into one or two non-wrap pieces.
--    Each piece is a sub-orbit of @s@ that doesn\'t cross @0@\/@2^w-1@,
--    so the operations below are well-defined on it.
--
-- 2. For each piece @p@:
--
--    a. 'liftForcedBits': consume the contiguous run of forced bits of
--       @b@ starting at @p@\'s stride boundary, doubling the stride and
--       halving the orbit per forced bit. Drops the piece if @p@\'s
--       'start' disagrees with @b@ on a low-bit position.
--    b. 'arcMeetClosed' against the unsigned arc @[blo, bhi]@ from
--       @b@\'s numeric bounds, intersecting the lifted piece with the
--       sub-arc @b@ admits.
--
-- 3. Combine the surviving pieces:
--
--    * Refined strides: 'compactify'. If the result is a single piece,
--      take it. If multiple pieces remain (@s@\'s wrap and @b@\'s arc
--      carve out two disjoint sub-arcs that aren\'t exactly representable
--      as one progression), return the whole-orbit 'liftForcedBits' of
--      @s@, which over-approximates the union without leaving @s@.
--    * Refined bitwise: @b@ ∧ ⋁ᵢ 'toBitwise' @pᵢ@. Per-piece 'toBitwise'
--      is bit-complete on non-wrap pieces (every bit constant across the
--      piece is detected); joining yields a bitwise domain dominated by
--      'toBitwise' @s@ and typically strictly tighter on wrap inputs.
--
-- == Why 'ssplit' first
--
-- 'refineByBits' alone runs the lift on the whole orbit and only then
-- 'ssplit's for the arc clip. When the lifted orbit happens to
-- self-wrap, the arc clip is skipped (it would over-approximate). By
-- splitting first we keep every piece non-wrap, so the per-piece arc
-- clip is always available — and at the same time the per-piece bitwise
-- projection beats projecting the whole orbit.
--
-- == Self-wrap fallback
--
-- On a self-wrapping @s@, 'ssplit' collapses to the full coset (a
-- /superset/ of @s@), which would break the subset guarantee. We fall
-- back to 'refineByBits' followed by 'refineBitsByStrides', which handle
-- self-wrap correctly (the lift is sound on any input, and
-- 'refineBitsByStrides' takes its own self-wrap branch).
reducePrecise ::
  (1 <= w) =>
  NatRepr w ->
  Domain w ->
  B.Domain w ->
  Maybe (Domain w, B.Domain w)
reducePrecise w s0 b0 = do
  (s1, b1) <- reducePrecisePass w s0 b0
  -- Second pass: the first pass produces a @b\'@ whose forced bits and
  -- numeric arc can be strictly tighter than @b@\'s. Re-running the
  -- pass propagates that information back to the strides side.
  -- A third pass /could/ tighten further (a smaller @s\'\'@ may have
  -- additional bits constant across pieces, which would force more bits
  -- in @b\'\'\'@), so two passes is not a fixpoint. Use 'reduceFixpoint'
  -- when full precision is required.
  reducePrecisePass w s1 b1

-- | A single pass of the algorithm described in 'reducePrecise'\'s
-- haddock — ssplit, per-piece lift + arc clip, recombine. Used by
-- 'reducePrecise', which runs it twice.
reducePrecisePass ::
  (1 <= w) =>
  NatRepr w ->
  Domain w ->
  B.Domain w ->
  Maybe (Domain w, B.Domain w)
reducePrecisePass w s b
  -- @b@ is empty: short-circuit before the @bhi - blo@ below would
  -- underflow on the canonical bottom @(blo = mask, bhi = 0)@.
  | B.isBottom b = Nothing
  -- See \"Self-wrap fallback\" in 'reducePrecise'\'s haddock.
  | isSelfWrapping s = do
      s' <- refineByBits w s b
      b' <- refineBitsByStrides w b s'
      Just (s', b')
  | otherwise =
      let zo = knownZerosOnesNat b
          (bloI, bhiI) = B.bitbounds b
          blo = integerToNatural bloI
          bhi = integerToNatural bhiI
          arc = mk w blo 1 (bhi - blo)
          -- Lift each non-wrap piece by @b@\'s forced bits, then
          -- intersect with @b@\'s numeric arc. Pieces wholly outside
          -- the arc, or whose low bits conflict with @b@, drop out.
          refined =
            [ p2
            | p  <- ssplit w s
            , Just p1 <- [liftForcedBits w p zo]
            , Just p2 <- [arcMeetClosed w p1 arc]
            ]
      in case refined of
           [] -> Nothing
           c : cs ->
             let -- Bitwise side: per-piece projection joined together,
                 -- then intersected with the input @b@.
                 stridesB = foldr (B.join . toBitwise) (toBitwise c) cs
                 b' = B.meet b stridesB
             in if B.isBottom b' then Nothing
                else
                  let -- Strides side: collapse pieces to a single
                      -- progression when possible; otherwise fall back
                      -- to the whole-orbit lift, which is a sound
                      -- subset of @s@ and avoids over-approximating to
                      -- a single-progression cover that would step
                      -- outside @s@.
                      s' = case compactify w refined of
                             [c'] -> c'
                             _    ->
                               case liftForcedBits w s zo of
                                 Just sl -> sl
                                 Nothing -> s
                  in Just (s', b')
...
-- | /'smoothPartOf' is sound and exact/: the returned @(mask, product)@ has
-- @product == 'oddProduct' mask@ (consistent encoding), @product@ divides the
-- input (it is a genuine divisor), the mask is within basis range, and
-- @product == gcd(m, 'basisProduct')@ — i.e. it is the /largest/ squarefree-basis
-- divisor (the @gcd@-against-@R@ characterization of plan critical-detail #4,
-- not mere divisibility testing).
smoothPartDividesAndSquarefree :: Natural -> Property
smoothPartDividesAndSquarefree m =
  property $
    let (msk, prod) = smoothPartOf m
    in prod == oddProduct msk
       && (m == 0 || m `mod` prod == 0)
       && msk < (1 `shiftL` basisSize)
       && prod == Prelude.gcd m basisProduct

-- | /'smoothGcd' is a sound common divisor/: for any positive strides, the
-- mask-AND gcd read off their 'strideFields' divides the true integer
-- @gcd@. (On smooth strides it is moreover /exact/; see 'smoothGcdExactOnSmooth'.)
smoothGcdDividesTrueGcd :: Natural -> Natural -> Property
smoothGcdDividesTrueGcd sa sb =
  sa > 0 ==> sb > 0 ==>
    property (g > 0 && Prelude.gcd sa sb `mod` g == 0)
  where g = smoothGcd (strideFields sa) (strideFields sb)

-- | /'smoothGcd' is exact on smooth strides/: when both operands are genuinely
-- smooth (@2^k · oddProduct mask@), the @min@-on-exponents AND-on-masks formula
-- equals the true integer @gcd@ — the AND-of-channels claim that replaces Euclid
-- on the lattice hot path.
smoothGcdExactOnSmooth :: Word -> Word -> Word -> Word -> Property
smoothGcdExactOnSmooth ka' kb' ma' mb' =
  property (Prelude.gcd sa sb == smoothGcd (ka, ma) (kb, mb))
  where
    bm = (1 :: Word) `shiftL` basisSize
    ka = ka' `mod` 32 ; kb = kb' `mod` 32
    ma = ma' `mod` bm ; mb = mb' `mod` bm
    sa = (1 `shiftL` fromIntegral ka) * oddProduct ma
    sb = (1 `shiftL` fromIntegral kb) * oddProduct mb

-- | /'smoothLcm' is exact on smooth strides/: when both operands are genuinely
-- smooth (@2^k · oddProduct mask@), the @max@-on-exponents OR-on-masks formula
-- equals the true integer @lcm@ — the dual of 'smoothGcdExactOnSmooth', backing
-- the combined stride in 'arcMeetClosed' on the meet hot path.
smoothLcmExactOnSmooth :: Word -> Word -> Word -> Word -> Property
smoothLcmExactOnSmooth ka' kb' ma' mb' =
  property (Prelude.lcm sa sb == smoothLcm (ka, ma) (kb, mb))
  where
    bm = (1 :: Word) `shiftL` basisSize
    ka = ka' `mod` 32 ; kb = kb' `mod` 32
    ma = ma' `mod` bm ; mb = mb' `mod` bm
...
-- | /'smoothCrtAlign' computes the CRT (Chinese Remainder Theorem) residue/:
-- for all exponents @ka@, @kb@ and odd-factor masks @ma@, @mb@ (giving smooth
-- strides @ta = 2^ka · oddProduct ma@, @tb = 2^kb · oddProduct mb@) and all
-- starts @sA@, @sB@, the result of @'smoothCrtAlign' (ka, ma) sA (kb, mb) sB@,
-- when @'Just' r@, is a value @r@ in @[0, lcm(ta,tb))@ congruent to @sA@ mod
-- @ta@ and to @sB@ mod @tb@; and it is 'Nothing' exactly when no common residue
-- exists (@sA ≢ sB@ mod @gcd(ta,tb)@). This is the soundness+completeness
-- contract that lets 'arcMeetClosed' swap the G-class 'solveLinearDiophantine'
-- alignment for the A\/M-class smooth prime-power recombination on the coprime
-- case.
smoothCrtAlignCorrect ::
  Word -> Word -> Word -> Word -> Natural -> Natural -> Property
smoothCrtAlignCorrect ka' kb' ma' mb' sa' sb' =
  property $
    case smoothCrtAlign (ka, ma) sA (kb, mb) sB of
      Nothing -> (toInteger sB - toInteger sA) `Prelude.mod` toInteger (Prelude.gcd ta tb) /= 0
      Just r  -> r < l
                   && (toInteger r - toInteger sA) `Prelude.mod` toInteger ta == 0
                   && (toInteger r - toInteger sB) `Prelude.mod` toInteger tb == 0
  where
    bm = (1 :: Word) `shiftL` basisSize
    ka = ka' `Prelude.mod` 32 ; kb = kb' `Prelude.mod` 32
    ma = ma' `Prelude.mod` bm ; mb = mb' `Prelude.mod` bm
    ta = (1 `shiftL` fromIntegral ka) * oddProduct ma
    tb = (1 `shiftL` fromIntegral kb) * oddProduct mb
    l  = Prelude.lcm ta tb
    sA = sa' `Prelude.mod` (l + 1)
    sB = sb' `Prelude.mod` (l + 1)

-- | /The cached smooth part divides the stride/: 'mk' preserves the exact
-- requested stride and caches its largest smooth divisor in the
-- @(stridePow2, oddFactorsMask)@ fields. That divisor, @2^stridePow2 ·
-- oddProduct oddFactorsMask@, always divides the stride (and equals it exactly
-- when the stride is itself smooth). This is the data-structure invariant since
-- 'mk' stopped forcing the stride to be smooth.
smoothPartDividesStride :: Domain w -> Property
smoothPartDividesStride c =
  proper c ==> property (r > 0 && stride c `Prelude.mod` r == 0)
  where
    r = (1 `shiftL` fromIntegral (stridePow2 c)) * oddProduct (oddFactorsMask c)
...
-- | /'mk' preserves every requested orbit point/: since 'mk' keeps the exact
-- requested stride (design C — it no longer rounds to a smooth stride), every
-- element of the requested orbit @{ s + i·st mod 2^w : 0 ≤ i ≤ nn }@ is a
-- 'member' of @'mk' w s st nn@. Inputs are clamped internally to the valid 'mk'
-- domain. (The smoothing /widening/ now lives in 'coarsenToSmooth'; its
-- over-approximation soundness is 'coarsenToSmoothSound'.)
mkSmoothingSound ::
  (1 <= w) => NatRepr w -> Natural -> Natural -> Natural -> Natural -> Property
mkSmoothingSound w s st nn i =
  property (member (mk w s' st' nn') ((s' + i' * st') .&. m))
  where
    m     = integerToNatural (maxUnsigned w)
    s'    = s .&. m
    st'   = max 1 (st .&. m)
    orbit = orbitLenOf m (lowestSetBit st')
    nn'   = nn `mod` orbit
    i'    = if nn' == 0 then 0 else i `mod` (nn' + 1)

-- | /'coarsenToSmooth' over-approximates/: widening the stride to its smooth
-- part (rounding the odd factors down, denser coset) yields a /superset/, so
-- every element of @c@ is a 'member' of @'coarsenToSmooth' w c@ (i.e.
-- @γ(c) ⊆ γ(coarsenToSmooth c)@). This is the widening-soundness witness for the
-- loose meet ('pseudoMeet'), which coarsens its operands so its A-class kernel is
-- sound; it is the role 'mk' smoothing played before the stride was kept exact.
coarsenToSmoothSound ::
  (1 <= w) => NatRepr w -> Domain w -> Natural -> Property
coarsenToSmoothSound w c i =
  proper c ==> property (member (coarsenToSmooth w c) elt)
  where
    i'  = if n c == 0 then 0 else i `mod` (n c + 1)
    elt = (start c + i' * stride c) .&. mask c
...
-- addSubSizeCorrect removed under smoothing: 'addRaw'\/'subRaw' now smooth their
-- result, so the closed-form 'addSubSize' (which does not model smoothing) no
-- longer equals @size . addRaw@ on the reversed (non-smooth) orientations this
-- property exercised. Reinstate if the orientation split lands and the scorer is
-- made smoothing-aware (or replaced by 'orientRobust' materialize-and-min).
...
-- addRobustClosedFormAgrees removed: 'add'\/'sub' are now single-orientation
-- ('addRaw'\/'subRaw'), so they no longer equal the orientation-robust
-- 'orientRobust' result. The 'orientRobust'\/'orientRobustAddSub'\/'addSubSize'\/
-- 'strideOrientations' machinery is retained (now unused) for a future
-- @addPrecise@\/@subPrecise@\/@mulPrecise@ that opts back into orientation
-- robustness; see 'add'.
...
-- mulConstExact removed under smoothing: scale-by-constant is no longer exact
-- (a non-smooth @k·stride@ is smoothed to a coarser coset; plan #3 accepts this
-- mul precision loss). 'mulConstExactCond'\/'exactScaleImage' are now unused.
...
-- | The contiguous-low-mask fast path in 'andSingleton' is sound: with
-- @k = 2^j - 1@ (the inputs that exercise cases (1)-(3) of the fast path,
-- not just the 'fromForcedBits' fallback), @(k & y) ∈ andSingleton w k c@ for
-- every @y ∈ c@.
andSingletonLowMaskSound ::
  (1 <= w) => NatRepr w -> Int -> Domain w -> Natural -> Property
andSingletonLowMaskSound w j c y =
  proper c ==> member c y ==>
    let j' = j `mod` (NR.widthVal w + 1)        -- 0 <= j' <= w
        k  = (1 `shiftL` j') - 1                -- contiguous low mask 2^j' - 1
    in property (member (andSingleton w k c) (k Bits..&. y))
...
pseudoMeetLowerBound _w a b =
  proper a ==> proper b ==> mask a == mask b ==>
    Prelude.not (wraps a) ==> Prelude.not (wraps b) ==>
      -- Smoothness precondition: on non-smooth strides 'pseudoMeet' coarsens its
      -- operands, so the over-approximation may step outside them (sound, but not
      -- a lower bound) — analogous to 'pseudoJoinUpperBound' being conditioned on
      -- non-wrapping. The genuine lower bound on any strides is 'lowerBound'.
      smooth a ==> smooth b ==>
...
-- TODO(smoothclp-cap): 'liftForcedBitsRefinesSpec' is DISABLED under smoothing.
-- The reference 'liftForcedBitsSpec' calls 'mk' (which now /smooths/ and
-- re-canonicalizes the stride) at /every/ bit-lift step, whereas the closed-form
-- 'liftForcedBits' calls 'mk' once at the end. With smoothing+capping these two
-- desynchronize: smoothing a partially-lifted stride mid-walk (e.g. a full-coset
-- canonicalization that drops the odd part) changes which bit the next step
-- lifts, so the bit-at-a-time spec and the closed form no longer denote the same
-- set. Counterexample at @w=5@: @s=(10,3,30)@, @zo=(19,12)@ gives @fast@ =
-- full-coset-of-8 @{4,12,20,28}@ but @ref@ = @{12,28}@, and @fast ⊄ ref@.
-- The closed form is still sound ('correct_refineByBits'), shrinking
-- ('liftForcedBitsShrinks') and forced-bit-correct ('liftForcedBitsMember'); only
-- its agreement with this particular executable spec is lost. Re-deriving a
-- smoothing-aware spec (or a smoothing-aware closed form) is future work.
--
-- liftForcedBitsRefinesSpec ::
--   (1 <= w) =>
--   NatRepr w -> Domain w -> B.Domain w -> Property
...
-- refineByBitsPreciseDominatesRoundTripNonSelfWrap removed: the cap on the
-- forced-bit lift (TODO(smoothclp-cap)) lowers strides-side precision, so
-- 'refineByBitsPrecise' no longer dominates the toBitwise\/fromBitwise
-- round-trip. (Dominance properties that a sound precision change falsifies are
-- dropped per the precision conventions.)
...
-- | 'reduce' is sound: when @x@ is in both input components, @x@ is in
-- both output components.
correct_reduce ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
...
-- | /Soundness of 'reduceProject'/: any value in both the strides orbit and the
-- bitwise component is in both outputs of the projection-only reduce. (Like
-- 'correct_reduce', but for the lift-free half — the orbit is returned unchanged,
-- so its membership is trivially preserved; the content is that the projected
-- bitwise component drops no shared value.)
correct_reduceProject ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
correct_reduceProject w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
    member s x ==> B.member b (toInteger x) ==>
      case reduceProject w s b of
        Nothing       -> property False
        Just (s', b') ->
          property (member s' x && B.member b' (toInteger x))

-- | 'reduce' only shrinks the strides component: every member of the
-- output strides component is a member of the input strides component.
reduceShrinksStrides ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
reduceShrinksStrides w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
    case reduce w s b of
      Nothing       -> property True
      Just (s', _)  ->
        member s' x ==> property (member s x)

-- | 'reduce' only shrinks the bitwise component: every member of the
-- output bitwise component is a member of the input bitwise component.
reduceShrinksBitwise ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
reduceShrinksBitwise w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
    case reduce w s b of
      Nothing       -> property True
      Just (_, b')  ->
        B.member b' (toInteger x) ==> property (B.member b (toInteger x))

-- | When 'reduce' returns 'Nothing', the joint really is empty:
-- no @x@ can be a member of both components.
reduceConflictMeansEmpty ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
...
-- | 'reducePrecise' is sound: when @x@ is in both input components,
-- @x@ is in both output components.
correct_reducePrecise ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
correct_reducePrecise w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
    member s x ==> B.member b (toInteger x) ==>
      case reducePrecise w s b of
        Nothing       -> property False
        Just (s', b') ->
          property (member s' x && B.member b' (toInteger x))

-- | 'reducePrecise' only shrinks the strides component.
reducePreciseShrinksStrides ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
reducePreciseShrinksStrides w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
    case reducePrecise w s b of
      Nothing       -> property True
      Just (s', _)  ->
...
-- | 'reducePrecise' only shrinks the bitwise component.
reducePreciseShrinksBitwise ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
reducePreciseShrinksBitwise w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
    case reducePrecise w s b of
      Nothing       -> property True
      Just (_, b')  ->
...
-- | When 'reducePrecise' returns 'Nothing', the joint is empty.
reducePreciseConflictMeansEmpty ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
reducePreciseConflictMeansEmpty w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
    case reducePrecise w s b of
      Just _  -> property True
      Nothing -> property (Prelude.not (member s x && B.member b (toInteger x)))

-- | 'reducePrecise'\'s strides component is at least as tight as
-- 'reduce'\'s: every member of @reducePrecise@'s output strides is
-- also a member of @reduce@'s.
reducePreciseDominatesReduceStrides ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
reducePreciseDominatesReduceStrides w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
    case (reduce w s b, reducePrecise w s b) of
      (_, Nothing)                   -> property True
      (Nothing, Just _)              -> property True
      (Just (sR, _), Just (sP, _))   ->
        member sP x ==> property (member sR x)

-- | 'reducePrecise'\'s bitwise component is at least as tight as
-- 'reduce'\'s.
reducePreciseDominatesReduceBitwise ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
reducePreciseDominatesReduceBitwise w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
    case (reduce w s b, reducePrecise w s b) of
      (_, Nothing)                   -> property True
      (Nothing, Just _)              -> property True
      (Just (_, bR), Just (_, bP))   ->
        B.member bP (toInteger x) ==> property (B.member bR (toInteger x))

-- | Re-running 'reduceFixpoint' on its own output produces a strides
-- component with the same members.
reduceFixpointIdempotentStrides ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
reduceFixpointIdempotentStrides w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
...
-- | Re-running 'reduceFixpoint' on its own output produces a bitwise
-- component with the same members.
reduceFixpointIdempotentBitwise ::
  (1 <= w) =>
  NatRepr w -> Domain w -> B.Domain w -> Natural -> Property
reduceFixpointIdempotentBitwise w s b x =
  proper s ==> toInteger (mask s) == B.bvdMask b ==>
    case reduceFixpoint w s b of
      Nothing       -> property True
      Just (s', b') ->
