...
import Who2.Expr.AbsVal (AbsVal, avTop)
...
computeTopAbsVal :: App t (Expr t (App t)) tp -> AbsVal tp
...
naiveAllocWithAbsVal :: TestBuilder t -> App t (Expr t (App t)) tp -> AbsVal tp -> IO (Expr t (App t) tp)
...
instance WI.IsSymExprBuilder (TestBuilder t) where
  ...
  varExpr _tb var =
    -- The TestBuilder always uses the unconstrained abstract value for bound
    -- variables; What4's 'bvarAbstractValue' hint is in a different domain.
    let absVal = avTop (WEA.bvarType var)
        app = BoundVarApp var
        expr = RiskyExpr
          { eId = WEA.bvarId var
