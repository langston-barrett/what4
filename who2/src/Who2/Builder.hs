...
checkSingleton app absVal = case E.baseType app of
  BT.BaseBoolRepr -> case (app, absVal) of
    ...
    _ -> True
  BT.BaseBVRepr _ -> case (app, AV.bvAsSingleton absVal) of
...
allocWithHashCons b app absVal = do
  ...
  case me of
    Just e -> do
      ...
      return $
        X.assert (AV.avCheckEq baseTypeRep (E.eAbsVal e) absVal /= Just False) e
    Nothing -> do
...
instance WI.IsSymExprBuilder (Builder t) where
  ...
  freshConstant b nm tp = do
    ...
    absVal <- case tp of
      WI.BaseBVRepr _ -> return $ AV.avTop tp
      WI.BaseBoolRepr -> return $ AV.avTop tp
  ...
  varExpr _b var =
    -- Always use the unconstrained (top) abstract value for bound variables.
    -- Who2 does not interpret What4's 'bvarAbstractValue' hint because the
    -- abstract domain is now strides, not What4's BVDomain.
    let absVal = AV.avTop (WEA.bvarType var)
