...
mkBoundVar cache var = do
  -- Bound-variable abstract-domain constraints are not emitted: 'WEA.BVar'
  -- carries What4's 'AD.AbstractValue', not the strides domain Who2 uses.
  lookupOrGenVarName cache (WE.bvarId var) (WEA.bvarType var)
...
applyBVAbstractConstraints w domain baseTerm
  ...
  | Just _ <- AV.bvAsSingleton domain = baseTerm
  | otherwise =
      let (forcedZeros, forcedOnes) = Str.forcedBits domain
          known1 = toInteger forcedOnes        -- bits that MUST be 1
          -- Bits that MUST be 0 (positions in forcedZeros). The mask we apply
          -- with bvand should have 1s where bits are allowed to be 1, i.e.
          -- complement of forcedZeros (within width @w@).
          mask = maxUnsigned w
          known0_mask = mask .&. complement (toInteger forcedZeros)
      in if forcedOnes == 0 && forcedZeros == 0
