...
import Who2.Expr.AbsVal (AbsVal, HasAbsVal(getAbsVal))
...
instance
  ...
  x == y
    | eId x == eId y = True
    | Just b <- AV.avCheckEq (baseType x) (eAbsVal x) (eAbsVal y) = b
    | hashConsing = False
    | eHash x /= eHash y = False
    | otherwise = eApp x == eApp y

instance HasAbsVal (Expr t f) where
  getAbsVal = eAbsVal
  {-# INLINE getAbsVal #-}
...
instance TestEquality (f (Expr t f)) => PC.TestEquality (Expr t f) where
  testEquality x y
    | hashConsing = testEquality (eId x) (eId y)
    | otherwise =
...
instance
  ...
  compare x y
    | hashConsing = compare (eId x) (eId y)
    | eId x == eId y = EQ
