...
asCoeff =
  \case
    SR.SemiRingIntegerRepr -> AD.asSingleRange . AV.getAbsVal
    SR.SemiRingRealRepr -> AD.asSingleRange . AD.ravRange . AV.getAbsVal
    SR.SemiRingBVRepr _ w -> fmap (BV.mkBV w) . AV.bvAsSingleton . AV.getAbsVal
{-# INLINE asCoeff #-}
...
add :: AV.HasAbsVal f => SRSum sr f -> SRSum sr f -> SRSum sr f
add ws1 ws2 =
  SRSum
