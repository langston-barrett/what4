...
isTrue e = E.eAbsVal e == Just True
{-# INLINE isTrue #-}

isFalse :: Expr t f BT.BaseBoolType -> Bool
isFalse e = E.eAbsVal e == Just False
{-# INLINE isFalse #-}
...
eq ::
  ...
  Expr t f BT.BaseBoolType ->
  (forall tp'. EL.LogicExpr (Expr t f) tp' -> AbsVal tp' -> IO (Expr t f tp')) ->
  Expr t f tp ->
  Expr t f tp ->
...
eq trueExpr falseExpr alloc x y
    ...
    -- test: bvnonzero-zero
  | x == y = pure trueExpr
  | Just b <- AV.avCheckEq (E.baseType x) (E.eAbsVal x) (E.eAbsVal y) =
...
ite alloc c t f
  ...
  | isFalse c = pure f
    ...
    -- test: bvite-same
  | t == f = pure t
  | otherwise = alloc (EL.Ite c t f) (AV.avJoin (baseType t) (E.eAbsVal t) (E.eAbsVal f))
{-# INLINE ite #-}
...
asBVLit' e = case AV.bvAsSingleton (E.eAbsVal e) of
...
bvUlt trueExpr falseExpr alloc x y
  ...
  | otherwise =
      let absVal = AV.bvUlt (E.eAbsVal x) (E.eAbsVal y)
      in case absVal of
...
bvUle trueExpr falseExpr alloc x y
  ...
  | otherwise =
      let absVal = fmap not (AV.bvUlt (E.eAbsVal y) (E.eAbsVal x))
      in case absVal of
           Just True -> pure trueExpr
           Just False -> pure falseExpr
...
bvSlt trueExpr falseExpr alloc x y
  ...
  | otherwise =
      let absVal = AV.bvSlt (bvWidth' x) (E.eAbsVal x) (E.eAbsVal y)
      in case absVal of
           Just True -> pure trueExpr
           Just False -> pure falseExpr
...
bvSle trueExpr falseExpr alloc x y
  ...
  -- x <= y iff not (y < x)
  | otherwise =
      let absVal = fmap not (AV.bvSlt (bvWidth' x) (E.eAbsVal y) (E.eAbsVal x))
      in case absVal of
           Just True -> pure trueExpr
           Just False -> pure falseExpr
